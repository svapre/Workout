...
Activity -> Routine -> Stage -> Plan -> Active runtime
...
- each entry rendered as a routine-owned wrapper around `ActivityCompact`
...
Must contain:
- stage name
- milestone or unlock state
- one-line stage goal

May contain:
- one short current, locked, or completed status marker
- one compact progress cue

Must not contain:
- routine names or routine strips
- cycle estimates
- full routine metadata
- activity metadata stacks
- long chapter prose

Detail-surface light path previews may omit the current/opening-stage goal when that same goal is already owned by the local owner module above.
...
- ordered schedule
- each step rendered as `StageStep`
- routine and rest steps sharing the same base shell
...
Must contain:
- plan overview
- opening-stage or adoption module
- compact roadmap
...
- duplicated opening-stage explanation in both the adoption module and the roadmap
- activity or routine detail inline
...
- routine or activity detail inline
...
New activity metadata appears on `ActivityDetail` first.
...
1. Activity detail and compact contract
