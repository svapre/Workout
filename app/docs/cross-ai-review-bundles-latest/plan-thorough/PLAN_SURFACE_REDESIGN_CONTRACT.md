...
- selected mobile nodes state the stage goal once, while the attached script starts directly with ordered steps
- unselected mobile stage nodes show an explicit selection cue on touch screens
...
- blueprint compare cards use sentence-safe truncation and user-facing labels such as Opening focus, not raw field names
