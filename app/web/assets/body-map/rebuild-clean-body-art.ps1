...
function Get-BackgroundColor {
  ...
  $r=0; $g=0; $b=0
  foreach($s in $samples){ $c=$Bitmap.GetPixel($s.x,$s.y); $r += $c.R; $g += $c.G; $b += $c.B }
  $count = $samples.Count
  return [pscustomobject]@{ R = [math]::Round($r/$count); G = [math]::Round($g/$count); B = [math]::Round($b/$count) }
}

function Build-CleanTransparentFigure {
  param(
    [string]$SourcePath,
    [string]$TargetPath,
    [double]$Low = 8,
  ...
  for ($x = 0; $x -lt $bmp.Width; $x++) {
    for ($y = 0; $y -lt $bmp.Height; $y++) {
      ...
      $blendToOriginal = [math]::Min(1, $alpha * 1.3)
      $blendToUnmix = 1 - $blendToOriginal

      $r = [math]::Round((($p.R / 255.0) * $blendToOriginal + $unmixR * $blendToUnmix) * 255)
      $g = [math]::Round((($p.G / 255.0) * $blendToOriginal + $unmixG * $blendToUnmix) * 255)
      $b = [math]::Round((($p.B / 255.0) * $blendToOriginal + $unmixB * $blendToUnmix) * 255)
...
Build-CleanTransparentFigure -SourcePath (Join-Path $assetDir 'neutral-front.png') -TargetPath (Join-Path $assetDir 'neutral-front-transparent.png')
Build-CleanTransparentFigure -SourcePath (Join-Path $assetDir 'neutral-back.png') -TargetPath (Join-Path $assetDir 'neutral-back-transparent.png')
